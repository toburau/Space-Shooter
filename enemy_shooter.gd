...
@export var attack_wait: float = 1.0
@export var rotate_speed: float = 0.1
@export var escape_speed: float = 500.0

enum State { kMove, kAttack, kRotate, kEscape }
	...
	match state:
		...
		State.kAttack:
			...
			if attack_timer <= 0:
				...
				state = State.kRotate
				escape_direction = (player.global_position - global_position).angle() + PI - PI/2
		State.kRotate:
			# プレイヤーと反対方向に回転
			rotation = rotate_toward(rotation, escape_direction, rotate_speed)
			if abs(rotation - escape_direction) < rotate_speed:
				state = State.kEscape
